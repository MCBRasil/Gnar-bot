	...
	public void parseResponse(Adventure adventure, Note n, String response) {
		...
		if (!(response.equalsIgnoreCase("pick it up") || response.equalsIgnoreCase("leave it") || response.equalsIgnoreCase("examine it") || response.equalsIgnoreCase("use it"))){
			...
			if (response.equalsIgnoreCase("leave it")) {
				super.sendMessage(n, "    :asterisk: You compel yourself to leave the Dildo, even though it looks massive and you want to try it, and you continue on your adventure.");
				adventure.getResponseFromEvent(this, "ignored");
				super.setCompletedEvent(true);
			}else if (response.equalsIgnoreCase("use it")) {
				adventure.sendMessage(n, "    :warning: You slowly creep towards it... with your heart beating faster.\n" +
						" You start to notice that your legs have been trembling ever since laying your eyes on it's shaft.\n" +
						" You poke it... breathing heavily. You notice it has a slight wobble, despite it being made of steel.\n" +
						" You hear a voice calling towards you *\"Ussseeee ittt. Usseeee itt! Use it and become one with me!\"*\n" +
						" You look around, seeing nobody, before lunging towards it, inserting it's long, cold but pleasing shaft into you.\n" +
						" :alarm_clock: *Time passes* You finally faint from the exhaustion of using it over and over again...");
				super.sendMessage(n, ":bulb: " +
						"Resting from using the dildo, you suddenly it shine.\n" +
						"However, in these woods, there's still no one around.\n" +
						"Yet, your heart throbs, the urge to do so is profound.\n" +
				...
				if (adventure.getInventory().addItem(new Item("Cracked Steel Dildo", "Dildo", "A Dildo that fell from the sky", 1)) != -1) {
					super.sendMessage(n, "        :asterisk: You pick up the Dildo and store it in your inventory.\n:bulb: `Use _adventure inventory to view your inventory!`");
					adventure.getResponseFromEvent(this, "stored");
					adventure.logAction("You picked up an item: Cracked Steel Dildo!");
				}else{
					super.sendMessage(n, "        :asterisk: You pick up the Dildo and attempt to store it, before realising that you have no room for it, so you leave it.\n" +
							":bulb: `Use _adventure inventory to view your inventory!`");
				...
				super.sendMessage(n, "        :asterisk: You examine the Dildo from afar.\n Upon further inspection, you seem to notice that it's a Cracked Steel Dildo." +
						"\n\n** Weapon Information: ** \n ➜ 3 Sex Drive \n ➜ No Bonus Effects" +
						"\n ➜ *\"A Dildo that fell from the sky while you were walking\"*");
