...
public class FirstSwordEvent extends Event {
    ...
    public Event runEvent(Adventure adventure, Message message) {
        super.sendMessage(message, ":bulb: *crash* Something lands in front of you." + "\n As the dust settles you see a "
                + "Sword stuck in the ground with it's hilt pointing upwards," + " and you feel some force bringing " +
                "you" + " closer to it." + "\n   :warning: What would you like to do?\n  " + "➜ `Pick it up`\n  ➜ " +
                "`Leave " + "it`\n  ➜ `Examine it`\n \n " + ":bulb: `Use the _adventure command to select a response!" +
                " Example: " + "_adventure Pick it up`");
        return this;
    }
    ...
    public void parseResponse(Adventure adventure, Message message, String response) {
        ...
        if (!(response.equalsIgnoreCase("pick it up") || response.equalsIgnoreCase("leave it") || response
            ...
            if (response.equalsIgnoreCase("leave it")) {
                super.sendMessage(message, "    :asterisk: You compel yourself to leave the sword, and you continue on " +
                        "your" + " adventure.");
                ...
                if (adventure.getInventory()
                        .addItem(new Item("Cracked Steel Sword", "sword", "A sword that fell from" + " the sky", 1))
                        != -1) {
                    super.sendMessage(message, "        :asterisk: You pick up the Sword and store it in your inventory" +
                    ...
                    super.sendMessage(message, "        :asterisk: You pick up the Sword and attempt to store it, before "
                ...
                super.sendMessage(message, "        :asterisk: You examine the sword from afar.\n Upon further inspection, " +
                        "you seem to notice that it's a Cracked Steel Sword." + "\n\n** Weapon Information: ** \n ➜ 3" +
                        " Damage \n ➜ No Bonus Effects" + "\n ➜ *\"A sword that fell from the sky while you were " +
                        "walking\"*");
