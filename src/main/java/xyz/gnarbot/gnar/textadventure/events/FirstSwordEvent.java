	...
	public void parseResponse(TextAdventure adventure, Note n, String response) {
		...
		if (!(response.equalsIgnoreCase("pick it up") || response.equalsIgnoreCase("leave it") || response.equalsIgnoreCase("examine it"))){
			...
			if (response.equalsIgnoreCase("leave it")){
				super.sendMessage(n, "    :asterisk: You compel yourself to leave the sword, and you continue on your adventure.");
				adventure.getResponseFromEvent(this, "ignored");
				super.setCompletedEvent(true);
			}else if (response.equalsIgnoreCase("pick it up")){
				if (adventure.getInventory().addItem(new Item("Cracked Steel Sword", "sword", "A sword that fell from the sky", 1)) != -1) {
					...
					super.sendMessage(n, "        :asterisk: You pick up the Sword and attempt to store it, before realising that you have no room for it, so you leave it.\n" +
							":bulb: `Use _adventure inventory to view your inventory!`");
