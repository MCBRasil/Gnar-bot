                ...
                try
                {
                    ...
                    if (count == 1)
                    {
                        eb.setTitle("**Quote from " + m.getAuthor().getName() + " (" + m.getCreationTime().format
                                (DateTimeFormatter.ISO_DATE_TIME) + ")**").addBlankField(true).setDescription(m
                                .getContent()).setThumbnail(m.getAuthor().getAvatarUrl()).setColor(new Color(0xFFDD15));
                    }
                    else
                    {
                        eb.addBlankField(true);
                        eb.addField("**Quote from " + m.getAuthor().getName() + " (" + m.getCreationTime().format
                                (DateTimeFormatter.ISO_DATE_TIME) + ")**", m.getContent(), false);
                    }
                }
                catch (Exception e)
                {
