...
public class YesNoPoll extends Poll
{
    ...
    public void startPoll()
    {
        ...
        final Note repliedMessage = n.replyRaw(":pushpin: *A new poll has been started by* **" + startingUser.getName
                () + "** `(Poll ID: " + getPollid() + ")`\n\n" + ":paperclip: Question:\n" + "        ╚ " + question
        ...
        runTask = Bot.INSTANCE.getScheduler().scheduleAtFixedRate(new Runnable()
        {
            ...
            public void run()
            {
                ...
                if (minutesInst >= 0)
                {
                    ...
                    {
                        repliedMessage.editMessage(":pushpin: *A new poll has been started by* **" + startingUser
                                .getName() + "** `(Poll ID: " + getPollid() + ")`\n\n" + ":paperclip: Question:\n" +
                                "        ╚ " + question + "\n\n" + ":clock1: Time Left:\n" + "        ╚ " +
                                minutesInst + " minute(s) " + seconds + " second(s)\n\n" + ":gem: Votes:\n" + "      " +
                                "  ╠ ❌ - No  [" + (repliedMessage.getReactions().get(0).getCount() - 1) + " Votes]\n"
                                + "        ╚ ✅ - Yes [" + (repliedMessage.getReactions().get(0).getCount() - 1) + " " +
                                "Votes]").queue();
                    }
                }
            }
        ...
        Bot.INSTANCE.getScheduler().schedule(() ->
        {
            ...
            repliedMessage.replyRaw(":exclamation: Poll `#" + getPollid() + "` by " + startingUser.getName() + " has " +
                    "finished! Check above for the results!");
