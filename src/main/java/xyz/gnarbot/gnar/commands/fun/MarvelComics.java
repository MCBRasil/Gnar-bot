...
public class MarvelComics extends CommandExecutor
{
    ...
    public void execute(Note message, String label, String[] args)
    {
        ...
        try
        {
            ...
            message.replyEmbedRaw("", " Here's your Marvel Character: " + StringUtils.capitalize(s.toLowerCase().replaceAll("\\+", " ")), Bot.getColor(), thumb.get("path") + "." + thumb.get("extension"), true);
        }
        catch (Exception e)
        {
            message.replyEmbed("", "*Couldn't find that character*", Bot.getColor());
        }
    }
}
